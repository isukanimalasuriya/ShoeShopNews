...
const EditShoeForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:5000/api/shoes/${id}`, dataToSend, {
        headers: {
      ...
      setErrors({ submit: error.response?.data?.error || 'Failed to update shoe' });
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
      <form onSubmit={handleSubmit} className="space-y-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">Edit Shoe</h2>
